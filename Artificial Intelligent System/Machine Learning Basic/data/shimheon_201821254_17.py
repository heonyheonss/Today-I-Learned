...
class EnvMod:
    ...
    def step(self, action, state):
        ...
        origin_idx = torch.argmax(state, dim=-1).int().numpy()[0] #원래 위치 반환
        reward = reward - 0.7 * (state_idx == origin_idx) #원래 위치와 행동 후 위치가 같다면 보상을 감한다.
        # 보상을 감하는 정도는 기존 학습에서 나타나는 행동 값이 0.8 전후로 나오므로, 0.7 씩 감하였다.
...
for e in range(EPISODES):
    ...
    while True:
        ...
        a = act_spc[a_idx[0]]
        state_, reward, done, info = myenv.step(a, state)
