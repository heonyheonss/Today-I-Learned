# -*- coding: utf-8 -*-
"""
Created on Mon Oct 30 14:55:07 2023

@author: heon
"""
import os
import datetime
import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.optim as optim
import torchvision
from torchvision import datasets, transforms
from torch.utils.data import TensorDataset, DataLoader
import numpy as np
from matplotlib import pyplot as plt

torch.manual_seed(1)

# %% Training Parameters
EPOCH = 10
BATCH = 100
LEARNINGRATE = 0.001

# %% MNIST Dataset

train_set = datasets.CIFAR10(
    root = './data/CIFAR10',
    train = True,
    transform = transforms.ToTensor(), # 데이터를 0에서 255까지 있는 값을 0에서 1사이 값으로 변환
    download = True
)
test_set = datasets.CIFAR10(
    root = './data/CIFAR10',
    train = False,
    transform = transforms.ToTensor(), # 데이터를 0에서 255까지 있는 값을 0에서 1사이 값으로 변환
    download = True
)

print('number of training data : ', len(train_set))
print('number of test data : ', len(test_set))
print(train_set[0][0].size())

classes = train_set.classes
print("classes :", classes)
num_classes = len(classes)
print("num_classes :", num_classes)

# %% data load
train_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=BATCH, shuffle=True)
test_loader = torch.utils.data.DataLoader(dataset=test_set, batch_size=BATCH, shuffle=True)

original_images, labels = next(iter(train_loader))
print("original_images.shape :", original_images.detach().numpy().shape)
# %% Plot some sample images
plt.figure(figsize=(10,4))
for i in range(10):
    plt.subplot(2, 5, i+1)
    img = train_set[i][0].numpy().transpose(1,2,0)
    plt.imshow(img)
    plt.axis('off')
    plt.title(classes[train_set[i][1]])
plt.show()

# %% Model define - class
class ConvBNRelu(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride = 1, padding = 1):
        super(ConvBNRelu, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size = kernel_size, stride = stride, padding = padding)
        self.batchnorm = nn.BatchNorm2d(out_channels)
    
    def forward(self, x):
        x = self.conv(x)
        x = self.batchnorm(x)
        x = F.relu(x)
        return x
    
class DenseBNRelu(nn.Module):
    def __init__(self, in_features, out_features):
        super(DenseBNRelu, self).__init__()
        self.dense = nn.Linear(in_features, out_features)
        self.batchnorm = nn.BatchNorm1d(out_features)
    
    def forward(self, x):
        x = self.dense(x)
        x = self.batchnorm(x)
        x = F.relu(x)
        return x
    
class CIFAR10Model(nn.Module):
    def __init__(self):
        super(CIFAR10Model, self).__init__()
        self.conv1 = ConvBNRelu(3, 32, kernel_size=3, padding=1)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.conv2 = ConvBNRelu(32, 64, kernel_size=3, padding=1)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.conv3 = ConvBNRelu(64, 128, kernel_size=3, padding=1)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.pool3_flat = nn.Flatten()
        self.dense4 = DenseBNRelu(128 * 4 * 4, 256)
        self.drop4 = nn.Dropout(p=0.4)
        self.dense5 = nn.Linear(256, 10)
        
    def forward(self, x):
        net = self.conv1(x)
        net = self.pool1(net)
        net = self.conv2(net)
        net = self.pool2(net)
        net = self.conv3(net)
        net = self.pool3(net)
        net = self.pool3_flat(net)
        net = self.dense4(net)
        net = self.drop4(net)
        net = self.dense5(net)
        return net

model = CIFAR10Model()
print(model)

# %% Optimizer
optimizer = optim.Adam(model.parameters(), lr=LEARNINGRATE)
lr_decay = 0.9
lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=len(train_set)/BATCH*5, gamma=lr_decay)

# loss
criterion = nn.CrossEntropyLoss()

# Train function
def train(model, images, labels):
    pred = model(images)
    loss = criterion(pred, labels)
    
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
    
    return loss

# Function to calculate accuracy
def accuracy_fn(Y, out):
    pred = torch.argmax(F.softmax(out, dim=1), 1)
    correct_prediction = (pred == Y)
    accuracy = correct_prediction.float().mean()
    return accuracy

# %% Checkpoints

checkpoint_dir = 'checkpoints'
model_dir = 'CIFAR10_model'
checkpoint_dir = os.path.join(checkpoint_dir, model_dir)
if not os.path.exists(checkpoint_dir):
    os.makedirs(checkpoint_dir)

# %% Load
def load(models, opimizers, checkpoint_file):
    print("[*] Reading checkpoints...")
    if os.path.exists(checkpoint_file):
        checkpoint = torch.load(checkpoint_file)
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        epoch = checkpoint['epoch']
        print("[*] Success to read checkpoint at epoch {}".format(epoch))
        return True, epoch
    else:
        print("[*] Failed to find a checkpoint")
        return False, 0

# %% Training
fstr = "\rEpch: {}, Loss: {:.6f}, Train accuracy: {:.4f}, Test accuracy: {:.4f}"

model.train()
start = datetime.datetime.now()
for epoch in range(EPOCH):
    avg_loss = 0.
    avg_train_acc = 0.
    avg_test_acc = 0.
    train_step = 0
    test_step = 0
    for images, labels in train_loader:
        avg_loss += train(model, images, labels)
        print('\r{:6.2f}'.format((train_step)/len(train_loader) * 100), end='')
        acc = accuracy_fn(labels, model(images))
        avg_train_acc += acc
        train_step += 1
    avg_loss = avg_loss / train_step
    avg_train_acc = avg_train_acc / train_step
    
    lr_scheduler.step()
    
    with torch.no_grad():
        for images, labels in test_loader:
            acc = accuracy_fn(labels, model(images))
            avg_test_acc += acc
            test_step += 1
            print('\r{:6.2f}'.format((test_step)/len(test_loader) * 100), end='')
        avg_test_acc = avg_test_acc / test_step
        
    print(fstr.format(epoch+1, avg_loss, avg_train_acc, avg_test_acc))
    
    checkpoint_file = os.path.join(checkpoint_dir, f'model_epoch_{epoch+1}.pth')
    checkpoint = {
        'epoch':epoch,
        'model_state_dict':model.state_dict(),
        'optimizer_state_dict':optimizer.state_dict(),
    }
    torch.save(checkpoint,checkpoint_file)
    
end = datetime.datetime.now()
print('Learning Finished!')
time = end-start
print("Learging Time: ",time)

# %% Load checkpoints
could_load, checkpoint_counter = load(model, optimizer, checkpoint_file)

if could_load:
    start_epoch = (int)(checkpoint_counter)+1
    print("[*] Load SUCCESS")
else:
    start_epoch = 0
    print("[!] Load failed...")
    
EPOCH = 13
start = datetime.datetime.now()
for epoch in range(start_epoch, EPOCH):
    avg_loss = 0.
    avg_train_acc = 0.
    avg_test_acc = 0.
    train_step = 0
    test_step = 0
    for images, labels in train_loader:
        avg_loss += train(model, images, labels)
        print('\r{:6.2f}'.format((train_step)/len(train_loader) * 100), end='')
        acc = accuracy_fn(labels, model(images))
        avg_train_acc += acc
        train_step += 1
    avg_loss = avg_loss / train_step
    avg_train_acc = avg_train_acc / train_step
    
    lr_scheduler.step()
    
    with torch.no_grad():
        for images, labels in test_loader:
            acc = accuracy_fn(labels, model(images))
            avg_test_acc += acc
            test_step += 1
            print('\r{:6.2f}'.format((test_step)/len(test_loader) * 100), end='')
        avg_test_acc = avg_test_acc / test_step
        
    print(fstr.format(epoch+1, avg_loss, avg_train_acc, avg_test_acc))
    
    checkpoint_file = os.path.join(checkpoint_dir, f'model_epoch_{epoch+1}.pth')
    checkpoint = {
        'epoch':epoch,
        'model_state_dict':model.state_dict(),
        'optimizer_state_dict':optimizer.state_dict(),
    }
    torch.save(checkpoint,checkpoint_file)
    
end = datetime.datetime.now()
print('Learning Finished!')
time = end-start
print("Learging Time: ",time)

# %% Validation
original_images, labels = next(iter(train_loader))
print("original_images.shape :", original_images.detach().numpy().shape)
out = F.softmax(model(original_images), dim=1)
preds = torch.argmax(out.data, 1)

for i in range(10):
    plt.subplot(2, 5, i+1)
    img = original_images[i].numpy().transpose(1,2,0)
    plt.imshow(img)
    plt.axis('off')
    plt.title(f"{classes[preds[i]]}\n{classes[labels[i]]}")
plt.show()
print("201821254 심헌")